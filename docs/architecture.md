...
本文档介绍 AiDoc 的整体架构，便于快速理解 Spring Boot 后端与 Vue Web 前端之间的协同关系。
...
- **核心模块**：
  ...
  - `DocumentationService`：封装模型调用（OpenAI 网关）与启发式兜底逻辑，始终返回结构化的 `DocGenerationResult`。
  - `DocService`：将生成记录及反馈写入 MySQL，配合 `TelemetryService` 做埋点。
  - `RateLimiterService`：基于 Redis 脚本的滑动窗口限流，自动降级到本地桶算法。
  - `SecurityConfig`：在 `/api/v1/**` 路径启用 API-Key 鉴权，与 Mintlify Writer API 兼容。
  - `AuthController` + `JwtAuthenticationFilter`：管理注册/登录，签发 JWT，拦截需要身份的 `/docs/**`、`/admin/**` 请求。
  - `AdminController`：提供用户角色/配额与 API-Key 管理接口，对应前端管理员面板。
- **AI 模型网关**：
  - `ModelGateway` 定义统一接口；
  ...
  - 配置位于 `application.yml -> ai.gateway.*`，可通过 `OPENAI_API_KEY` / `DEEPSEEK_API_KEY` 等变量注入。
- **数据流**：
  ...
  6. 服务启动时会扫描 `doc_jobs` 中 PENDING/IN_PROGRESS 的任务并重新派发。
...
- **MySQL**：存储 API-Key 与文档记录；见 `src/main/resources/schema.sql`。
- **Users/Doc Jobs**：`users` 表保存凭证与角色；`doc_jobs` 表持久化异步任务状态。
- **Redis**：限流与未来的分布式任务排队；通过 `spring.data.redis.*` 配置。
- **OpenAI**（可选）：配置 `AI_GATEWAY_*` 环境变量后启用真实模型；未配置时自动回退。
